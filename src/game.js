...
class Game {
  constructor(assets, data) {
    this.assets = assets; this.data = data; this.ui = new UI(); this.save = loadSave();
    ...
    this.bindCommands(); this.flow = new FlowController(this); document.addEventListener('visibilitychange', () => { if (document.hidden && this.state === STATES.GAMEPLAY && !this.victory) this.setPause('pause'); });
  ...
  bindCommands() {
    ...
    this.ui.renderBindings(this.input, false); this.ui.renderBindings(this.input, true);
  }
  solids() {
  ...
  resolvePlayerAttack(box, player) {
    ...
    for (const enemy of this.enemies) {
      ...
      if (enemy.takeDamage(player.attackStage === 3 ? 3 : 2, player.x, this)) { player.attackHit.add(enemy); player.energy = clamp(player.energy + 6, 0, player.maxEnergy); this.hitStop = Math.max(this.hitStop, 0.045); }
    }
    if (this.boss.active && !this.boss.dead && !player.attackHit.has(this.boss) && overlap(box, this.boss.hitbox())) {
      if (this.boss.takeDamage(player.attackStage === 3 ? 4 : 2, player.x, this)) { player.attackHit.add(this.boss); player.energy = clamp(player.energy + 5, 0, player.maxEnergy); }
    }
  }
  ...
  updateProjectiles(dt) {
    for (const projectile of this.projectiles) {
      ...
      if (projectile.type === 'ring' && projectile.age >= projectile.returnAt) {
        ...
        if (distance < 12) { projectile.x = socket.x; projectile.y = socket.y; this.endProjectile(projectile, .3); this.addFx('hit', socket.x, socket.y, .22); continue; }
      }
  ...
  retry() {
    this.ui.hideAll(); this.gameOver = false; this.projectiles.length = 0; this.effects.length = 0; this.hitStop = 0; this.movingY = 148; this.movingDirection = -1; this.movingDelta = 0; this.phaseTwo = false; this.arenaPlatforms.reset(); this.bossActive = false; this.courtyardClear = false; this.windRingCooldown = 0;
  ...
  setPause(reason = 'pause') { if (this.gameOver || this.victory || this.state !== STATES.GAMEPLAY) return; this.pauseReason = reason; this.overlayReturnState = STATES.GAMEPLAY; this.setState(STATES.PAUSED); this.ui.show(reason); }
  resume() {
  ...
  openOverlay(reason) {
    ...
    if (this.state === STATES.GAMEPLAY) this.setState(STATES.PAUSED); this.ui.show(reason);
  }
  ...
  applyDebugStart() {
    ...
    if (this.debugArea === 'boss' || this.debugArea === 'boss-center') {
      this.player.respawn({ x: this.debugArea === 'boss-center' ? 960 : 885, y: 176, facing: 'right' }); this.checkpoint = { x: 830, y: 176 }; this.cameraX = 768; this.bossActive = true; this.boss.activate();
      if (this.debugPhase >= 2) this.boss.hp = this.boss.maxHp * .5;
    }
  }
  enterGameplay(fromContinue = false, showChapter = false) {
    if (fromContinue) this.resetGameplayFromSave(); this.ui.hideAll(); this.modalOpen = null; this.pauseReason = null; this.flow.showOnly(showChapter ? this.flow.chapter : null);
  ...
  prepareMainMenu() { writeSave(this.save); this.projectiles.length = 0; this.effects.length = 0; this.bossActive = false; this.boss.active = false; this.pauseReason = null; this.modalOpen = null; this.ui.hideAll(); }
  ...
  finishBoss() {
    ...
    this.victory = true; this.setState(STATES.VICTORY); this.ui.show('victory'); this.input.clear();
  }
  update(dt) {
    this.time += dt; if (this.state === STATES.GAMEPLAY) this.runTime += dt; this.ui.update(dt, this);
    ...
    if (this.movingY < moving.min || this.movingY > moving.max) { this.movingY = clamp(this.movingY, moving.min, moving.max); this.movingDirection *= -1; }
    ...
    const platformEvent = this.arenaPlatforms.update(dt); if (platformEvent === 'destroyed') { const center = this.arenaPlatforms.center(); this.addFx('death', center.x + center.w / 2, center.y + 8, .5); }
    ...
    if (this.windRingCooldown <= 0 && rings.some((ring) => overlap(this.player.hitbox(), { x: ring.x + 8, y: ring.y + 8, w: (ring.width || 64) - 16, h: (ring.height || 64) - 16 }))) {
      this.player.vy = -205; this.player.dashAvailable = true; this.windRingCooldown = 0.55; this.addFx('reflect', this.player.x, this.player.y - 24, 0.3); this.ui.notify('風環重置翼環', 1);
    }
    this.enemies.forEach((enemy) => enemy.update(dt, this)); this.enemies = this.enemies.filter((enemy) => !enemy.remove);
    this.courtyardClear = !this.enemies.some((enemy) => !enemy.dead && enemy.x > 380 && enemy.x < 760);
    ...
    if (!this.boss.active && overlap(this.player.hitbox(), bossTrigger)) { this.bossActive = true; this.boss.activate(); this.ui.notify('天門守機「奧羅」'); }
...
async function boot() {
  ...
  try {
    ...
    globalThis.__SKY_CITY_GAME__ = new Game(assets, data);
