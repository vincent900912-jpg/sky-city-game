...
const defaults = {
  version: 4, level01Complete: false, introCompleted: false, checkpoint: null, unlockedSkills: ['windPulse'],
  maxHp: 10, maxEnergy: 100, controls: { ...DEFAULT_CONTROLS },
  settings: { volume: 0.8, shake: true }, bestTime: null,
};

function migrate(parsed) {
  if (!parsed || ![1, 2, 3, 4].includes(parsed.version)) return null;
  return {
    ...structuredClone(defaults), ...parsed, version: 4,
...
export function writeSave(data) {
  try {
    ...
    localStorage.setItem(KEY, JSON.stringify({ ...data, version: 4 })); return true;
...
export function hasValidSave() {
  for (const key of [KEY, BACKUP]) { try { const parsed = JSON.parse(localStorage.getItem(key)); if ([1, 2, 3, 4].includes(parsed?.version)) return true; } catch {} }
  return false;
}
export function resetForNewGame(save) {
  Object.assign(save, { version: 4, level01Complete: false, introCompleted: false, checkpoint: null, unlockedSkills: ['windPulse'], maxHp: 10, maxEnergy: 100, bestTime: null });
